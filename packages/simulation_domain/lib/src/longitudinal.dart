...
class LongitudinalPatientTrajectory {
  const LongitudinalPatientTrajectory(
      {required this.patientId, required this.epochs});
  final String patientId;
...
class LongitudinalSimulation {
  ...
  Map<String, Object?> coverageSummary() {
    ...
    var changes = 0;
    var recoveries = 0;
    var prolongedMissingness = 0;
    for (final trajectory in trajectories) {
      for (var i = 1; i < trajectory.epochs.length; i++) {
        final previous = trajectory.epochs[i - 1];
        final current = trajectory.epochs[i];
        if (previous.source != current.source) transitions++;
        if (current.pattern == LongitudinalPattern.drift ||
            current.pattern == LongitudinalPattern.stepChange ||
            current.pattern == LongitudinalPattern.regression) changes++;
        if (current.pattern == LongitudinalPattern.recovery) recoveries++;
        if (previous.state == SyntheticDataState.missing &&
            current.state == SyntheticDataState.missing) prolongedMissingness++;
      }
    }
    result['changes'] = changes;
    result['recoveries'] = recoveries;
    result['prolongedMissingness'] = prolongedMissingness;
    result['conflicts'] = epochs
  ...
  LongitudinalSimulation generate(
    ...
    for (var index = 0; index < cohort.patients.length; index++) {
      ...
      for (var sequence = 0; sequence < epochCount; sequence++) {
        final startsAt = _addMonths(start, sequence * 2);
        final endsAt = _addMonths(start, (sequence + 1) * 2);
        final pattern = LongitudinalPattern
            .values[(index + sequence) % LongitudinalPattern.values.length];
        final stateIndex = (index + sequence) % 18;
        final state = (stateIndex == 0 || stateIndex == 1)
    ...
    final simulation = LongitudinalSimulation(
        seed: seed, startedAt: start, years: years, trajectories: trajectories);
...
DateTime _addMonths(DateTime value, int months) {
  ...
  return DateTime.utc(year, month, day, value.hour, value.minute, value.second,
